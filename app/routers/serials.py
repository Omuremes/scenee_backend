...
@admin_router.post("/episodes/{episode_id}/poster", response_model=SerialEpisodeResponse)
async def upload_episode_poster(
    episode_id: UUID,
    poster: UploadFile = File(...),
    _current_admin: User = Depends(get_current_admin_user),
    db: AsyncSession = Depends(get_db)
    ...
    episode = await service.update_episode(episode_id, SerialEpisodeUpdate(poster_key=poster_key))
    return SerialEpisodeResponse.model_validate(episode)
