...
class BaseContentRepository(BaseRepository[Movie]):
    ...
    async def count_content(
        ...
        stmt = self._apply_filters(select(func.count(Movie.id)), query, category_id)
    ...
    async def get_new_content(self, limit: int = 10) -> List[Movie]:
        review_counts = await self._review_count_subquery()
        now = datetime.utcnow()
        freshness_rank = case(
    ...
    async def update_content_with_relations(
        self,
        movie_id: UUID,
        movie_data: dict,
        *,
    ...
    async def delete_content(self, movie_id: UUID) -> bool:
        movie = await self.get_content_by_id(movie_id)
        if not movie:
            return False
...
class ContentRepository(BaseRepository[Movie]):
