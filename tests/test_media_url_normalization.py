...
from app.schemas import ActorCreate, ActorUpdate, EventCreate, EventUpdate
from app.core import minio as minio_module
from app.core.minio import normalize_media_fields
from app.routers.events import _event_media_url
...
def test_event_urls_are_normalized_before_storage(monkeypatch):
    ...
    event_update = EventUpdate(poster_url="http://minio:9000/cinescope-media/events/event.jpg")

    assert event_create.image_url == "http://192.168.68.150:9000/cinescope-media/events/event.jpg"
    assert event_update.poster_url == "http://192.168.68.150:9000/cinescope-media/events/event.jpg"
    assert event_update.storage_path == "events/event.jpg"


def test_event_response_uses_storage_path_over_stale_absolute_url(monkeypatch):
    monkeypatch.setattr(minio_module.settings, "MINIO_ENDPOINT", "minio:9000")
    monkeypatch.setattr(minio_module.settings, "MINIO_PUBLIC_BASE_URL", "http://192.168.68.150:9000")

    event = SimpleNamespace(
        storage_path="events/posters/expo-night.jpg",
        poster_url="http://10.0.0.12:9000/cinescope-media/events/posters/expo-night.jpg",
        image_url="http://10.0.0.12:9000/cinescope-media/events/posters/expo-night.jpg",
    )

    assert _event_media_url(event) == "http://192.168.68.150:9000/cinescope-media/events/posters/expo-night.jpg"


def test_response_models_preserve_already_public_urls():
    minio_module.settings.MINIO_ENDPOINT = "minio:9000"
    minio_module.settings.MINIO_PUBLIC_BASE_URL = "http://192.168.68.150:9000"
    ...
    assert _event_media_url(event) == public_url
